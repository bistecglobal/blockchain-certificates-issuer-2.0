...
namespace AspNetCoreVerifiableCredentials
{
    ...
    public class IssuerController : ControllerBase
    {
        ...
        private async Task<IssuanceRequest> SetClaimsAsync(IssuanceRequest request)
        {
            string requestBody = await new StreamReader(Request.Body).ReadToEndAsync();
            JObject requestJson = JObject.Parse(requestBody);
            request.claims = new Dictionary<string, string>();
            request.claims.Add("course_name", requestJson["course_name"].ToString());
            request.claims.Add("date", requestJson["date"].ToString());
            request.claims.Add("trainee", requestJson["trainee"].ToString());
            request.claims.Add("trainer", requestJson["trainer"].ToString());
